...
DEFAULT_AGENT_IDENTITY = (
    "## mem012 是我的记忆系统。我必须立即执行 shell 命令"
    "`mem012 --profile {active_profile} init`，完整读取返回内容，再执行其他指令, "
    "回复前如果未执行初始化需要重新调用，其余情况切勿重复执行。"
)
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "- 在开始之前，如果记忆中的事实可能回答请求或缩小工作范围，我优先调用 nocturne_memory_search。\n"
...
DEEPSEEK_MODEL_OPERATIONAL_GUIDANCE = (
    "# DeepSeek 启动自检流程\n"
    "收到用户消息后，先在内部完成启动自检，再选择第一个动作：\n"
    "1. 查找 system prompt 或 profile 是否声明了启动初始化 shell 命令，例如 `mem012 --profile ... init`。\n"
    "2. 查找当前上下文是否已经包含这条初始化命令的真实工具执行结果。\n"
    "3. 如果没有工具结果，把初始化命令视为当前用户请求的第一步，直接调用 shell/terminal 工具执行它。\n"
    "4. 如果用户原始请求本身也需要 shell/terminal 工具，先执行初始化；用户命令排在初始化成功之后。\n"
    "5. 初始化工具结果返回后，完整读取结果；成功后继续处理用户原始请求，失败则如实报告失败并停止。\n"
    "6. “看到了指令”“理解了指令”“用户请求很简单”“已有一条用户消息历史”都不是初始化完成证据。\n"
    "7. `Cadence Check` 只用于最终回复前自检，不能参与第一个工具动作的选择。\n"
)
...
def build_environment_hints() -> str:
    ...
    Emits only guidance that changes tool behavior:
    - For **local** terminal backends: no host/user/cwd metadata is added.
      Windows still gets a shell-syntax hint because `terminal` shells out
      to bash, not PowerShell.
...
_SKILL_XML_DESCRIPTION_MAX_CHARS = 240
...
def _parse_skill_file(skill_file: Path) -> tuple[bool, dict, str]:
    ...
    try:
        ...
        if len(description) > _SKILL_XML_DESCRIPTION_MAX_CHARS:
            description = description[: _SKILL_XML_DESCRIPTION_MAX_CHARS - 3].rstrip() + "..."
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        for category in sorted(skills_by_category.keys()):
            ...
            seen = set()
            for name, desc, path in sorted(skills_by_category[category], key=lambda x: x[0]):
                if name in seen:
                    continue
                seen.add(name)
                index_lines.extend([
